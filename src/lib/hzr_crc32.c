...
static void cpuid(
    unsigned func, unsigned* a, unsigned* b, unsigned* c, unsigned* d) {
#if defined(__GNUC__) || defined(__clang__)
...
static uint32_t crc32c_sse4_2(const uint8_t *buf, size_t size) {
...
static uint32_t crc32c_fallback(const void* data, size_t length) {
  uint32_t crc = ~0;
...
#ifdef HZR_USE_SSE4_2
  if (can_use_sse4_2()) {
    return crc32c_sse4_2(data, length);
  }
#endif /* HZR_USE_SSE4_2 */
  return crc32c_fallback(data, length);
}
